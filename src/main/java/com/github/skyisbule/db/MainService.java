...
public class MainService {
    ...
    void createTable(String dbName, String tableName, List<String> names, List<ColumnTypeEnum> types)
        throws IOException {
    ...
    public LinkedList<ArrayList<String>> doSelect(String dbName, String tableName, int page) {
        return engine.doReadPage(dbName, tableName, page);
    }
                ...
                for (int column = 0; column < record.size(); column++) {
                    ...
                    if (filter.isDoFilter()) {
                        switch (filter.getType()) {
                            case EQUALS:
                                if (filter.getTarget().equals(str)) { pass.add(record); }
                                break;
                            case IN:
                                if (str.contains(filter.getTarget())) { pass.add(record); }
                                break;
                            case LESS:
                                if (Integer.parseInt(str) < filter.getVal()) { pass.add(record); }
                                break;
                            case THAN:
                                if (Integer.parseInt(str) > filter.getVal()) { pass.add(record); }
                                break;
